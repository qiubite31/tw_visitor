...
def parse_data_sheet3():
    ...
    return (pd.DataFrame(data_list, visitor_reason, visitor_area,), continent_mapping)

def parse_data_report():
    pass
...
xls_path = 'visitor_data/2018/201812.xls'
wb = open_workbook(xls_path, formatting_info=True)
sheet = wb.sheet_by_name('Sheet3')
sheet3 = pd.read_excel(xls_path, sheetname='Sheet3', header=1)
...
visitor_df = parse_data_sheet3()[0]
continent_mapping = parse_data_sheet3()[1]
...
for row in rows:
    for idx in range(0, row[1].size):
        ...
        value = row[1][idx]
        insert_record(year, month, continent, area_cht, area_eng,
                      purpose_cht, purpose_eng, value)
